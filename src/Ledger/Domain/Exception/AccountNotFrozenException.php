...
final class AccountNotFrozenException extends \RuntimeException
{
    public static function for(AccountId $accountId, AccountStatus $status): self
    {
        return new self(sprintf("Account %s is not frozen. Current status is %s.", $accountId->value, $status->name));
    }
}
